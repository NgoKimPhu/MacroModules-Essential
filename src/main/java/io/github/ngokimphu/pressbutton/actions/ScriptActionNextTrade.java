...
public class ScriptActionNextTrade extends ScriptAction {

    /**
     * Initialization
     */
    public ScriptActionNextTrade() {
        // Context is the context for this action, action name must be lowercase
        super(ScriptContext.MAIN, "nexttrade");
    }
    ...
    public IReturnValue execute(IScriptActionProvider provider, IMacro macro, IMacroAction instance, String rawParams,
            String[] params) {
        provider.actionAddChatMessage("§bBegin~");

        GameSettings gameSettings = this.mc.gameSettings;
        KeyBinding invKeyBinding = gameSettings.keyBindInventory;
        int invKeyCode = invKeyBinding.getKeyCode();
        KeyBinding.onTick(invKeyCode);
        if (!invKeyBinding.isKeyDown())
            KeyBinding.setKeyBindState(invKeyCode, true);

        provider.actionAddChatMessage("§cMid");
        ...
        provider.actionAddChatMessage("§bEnd");
        return null;
    }
